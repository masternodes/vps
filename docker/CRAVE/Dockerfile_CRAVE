#
# Dockerfile for a CRAVE masternode
# usage: docker run marsmensch/masternode-crave:latest
# 
# how to work with the container
# 
# 1) build the container (checkout desired git revision first)
# docker build -t "marsmensch/masternode-crave:latest" -f docker/Dockerfile_BSD .
#
# 2) start the container  
# docker run -p ${NODE_PORT}:${NODE_PORT} -v $(PWD)/config/crave/craved.conf:/opt/data "marsmensch/masternode-crave:latest"
# 
# 3) start the container interactively 
# docker run --interactive --tty --entrypoint=/bin/bash "marsmensch/masternode-crave:latest"
#
# 4) standard help
# docker run -v $(PWD)/config/crave:/opt/data "marsmensch/masternode-crave:latest"
...
ENV PROJECT          crave
ENV GIT_URL          git://github.com/CooleRRSA/crave.git
ENV SVC_VERSION      tags/v2.1.0.0
