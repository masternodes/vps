#
# Dockerfile for a SIB masternode
# usage: docker run marsmensch/masternode-sib:latest
# 
# how to work with the container
# 
# 1) build the container (checkout desired git revision first)
# docker build -t "marsmensch/masternode-sib:latest" -f docker/Dockerfile_SIB .
#
# 2) start the container
# docker run -p ${NODE_PORT}:${NODE_PORT} -v $(PWD)/config/sib/sibd.conf:/opt/data "marsmensch/masternode-sib:latest"
# 
# 3) start the container interactively 
# docker run --interactive --tty --entrypoint=/bin/bash "marsmensch/masternode-sib:latest"
#
# 4) standard help
...
ENV PROJECT          sibcoin
ENV GIT_URL          git://github.com/ivansib/sibcoin.git
ENV SVC_VERSION      tags/v0.16.1.2
...
# start command
CMD ["/usr/local/bin/sibcoind", "--help"]