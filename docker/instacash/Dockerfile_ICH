...
# docker build -t "marsmensch/masternode-pivx:latest" -f docker/Dockerfile_ICH.
#
...
#
# 4) standard help
# docker run -v $(PWD)/config/instacash:/opt/data "marsmensch/masternode-pivx:latest"

FROM                 ubuntu:xenial
MAINTAINER 			 Florian Maier <contact@marsmenschen.com>

ENV CONTAINER_USER   masternode
ENV PROJECT          instacash
ENV GIT_URL          git://github.com/insta-cash/instacash.git
ENV SVC_VERSION      tags/v1.1
...
# start command
CMD ["/usr/local/bin/instacashd", "--help"]
