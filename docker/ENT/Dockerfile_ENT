...
#
# 4) standard help
# docker run -v $(PWD)/config/ent:/opt/data "marsmensch/masternode-ent:latest"

FROM                 ubuntu:xenial
MAINTAINER 			 Florian Maier <contact@marsmenschen.com>

ENV CONTAINER_USER   masternode
ENV PROJECT          eternity
ENV GIT_URL          git://github.com/eternity-group/eternity.git
ENV SVC_VERSION      tags/v0.12.0.61
...
# start command
CMD ["/usr/local/bin/eternityd", "--help"]