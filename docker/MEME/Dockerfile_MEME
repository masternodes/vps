#
# Dockerfile for a MEME masternode
...
# docker build -t "marsmensch/masternode-meme:latest" -f docker/Dockerfile_MEME .
#
# 2) start the container 
# docker run -p ${NODE_PORT}:${NODE_PORT} -v $(PWD)/config/meme/memed.conf:/opt/data "marsmensch/masternode-meme:latest"
# 
# 3) start the container interactively 
# docker run --interactive --tty --entrypoint=/bin/bash "marsmensch/masternode-meme:latest"
#
# 4) standard help
# docker run -v $(PWD)/config/meme:/opt/data "marsmensch/masternode-meme:latest"
...
ENV PROJECT          meme
ENV GIT_URL          git://github.com/pepeteam/pepecoin.git
ENV SVC_VERSION      tags/2.4.3.2
...
RUN apt-get update \
    ...
    && cd src && make -f makefile.unix USE_UPNP= && cp pepecoind /usr/local/bin/pepecoind \
...
# start command
CMD ["/usr/local/bin/pepecoind", "--help"]