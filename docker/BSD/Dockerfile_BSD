#
# Dockerfile for a BSD masternode
# usage: docker run marsmensch/masternode-bsd:latest
# 
# how to work with the container
# 
# 1) build the container (checkout desired git revision first)
# docker build -t "marsmensch/masternode-bsd:latest" -f docker/Dockerfile_BSD .
#
# 2) start the container 
# docker run -p ${NODE_PORT}:${NODE_PORT}  -v $(PWD)/config/bitsend/bitsend.conf:/opt/data "marsmensch/masternode-bsd:latest"
# 
# 3) start the container interactively 
# docker run --interactive --tty --entrypoint=/bin/bash "marsmensch/masternode-bsd:latest"
#
# 4) standard help
# docker run -v $(PWD)/config/bitsend:/opt/data "marsmensch/masternode-bsd:latest"
...
ENV PROJECT          bitsend
ENV GIT_URL          git://github.com/LIMXTEC/BitSend.git
ENV SVC_VERSION      master
...
# start command
CMD ["/usr/local/bin/bitsendd", "--help"]